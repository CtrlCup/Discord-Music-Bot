...
`cogs/music.py` is a legacy/basic music implementation **not loaded** by the bot — `music_advanced.py` superseded it. `cogs/bug.py` (a `!bug` report command that DMs the application owner with the reporter's rolling action log) is likewise **not loaded** — fully implemented but absent from `cog_files`. `stats.py` (message/voice tracking, leaderboard) and `statistics_advanced.py` (music-listening stats) both load and coexist — they track different things, not duplicates. Don't assume a `cogs/*.py` file is live just because it exists; check the load list in `bot.py` first.

**Permission system** (`utils/permissions.py`): a three-tier hierarchy — User < Supporter < Admin — layered on top of Discord's own permissions, configured per-guild via `!settings setrole <user|supporter|admin> @Role` / `!settings removerole` (`cogs/settings.py`, itself gated `@is_admin_check()`) and stored as `user_role_id`/`supporter_role_id`/`admin_role_id` columns on `guild_settings`. Commands are gated with the `@is_user_check()` / `@is_supporter_check()` / `@is_admin_check()` decorators (used throughout `music_advanced.py`, `playlists.py`, `stats.py`, `account.py`, `info.py`); the underlying `has_role_or_higher()` walks the hierarchy top-down (Admin role or Discord's own `administrator`/`manage_guild` permission always satisfies any tier) and, critically, **falls back to an unconfigured tier being open to everyone** — an empty `user_role_id` means any member can use User-tier commands, empty `supporter`/`admin` restrict that tier to actual Discord admins/manage-guild holders instead of locking it out entirely. When `ctx.guild` is `None` (DM invocation), `has_role_or_higher()` checks the invoker's roles across every guild the bot shares with them (`ctx.bot.guilds`) rather than a single guild — this is what lets permission-gated commands be invoked from DMs, though the command body still needs a real `ctx.guild` for anything guild-scoped (queues, playlists, settings).

**Every command is a `commands.hybrid_command`/`hybrid_group`** (not `commands.command`/`commands.group`) — this makes each one work simultaneously as the classic `!prefix` text command and as a native Discord `/slash` command, without duplicating logic. When adding a new command, use the hybrid decorator, not the plain one, and keep the docstring (it becomes the slash command's description) **under 100 characters** — `tree.sync()` in `bot.py::_sync_app_commands()` rejects the whole batch if any description is too long. `bot.py::setup_hook()` calls `_sync_app_commands()` after cogs load: if `.env` sets `DEV_GUILD_ID`, commands sync only to that one guild (instant, and any previously-registered global commands are explicitly cleared first to avoid Discord showing every command twice); otherwise it syncs globally (correct for production, but can take up to an hour to propagate). Never sync both global and guild-specific for the same guild without clearing one first — Discord treats them as separate command objects and lists both.
