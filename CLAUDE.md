...
- `GuildMusicState` (in `music_utils.py`) holds per-guild state and supports **multiple simultaneous voice channels per guild** — queues are keyed per-channel via `GuildMusicState.get_queue(channel_id)`, not just per-guild. This is deliberate (`music.multi_channel_support` in config).
